...
/**
 * Reusable MongoDB Atlas connection helper.
 *
 * The connection string is read from `process.env.MONGODB_URI` so that no
 * MongoDB Atlas credentials are ever hardcoded in source. The connection is
 * established once at application startup.
 *
 ...
 * NOT call `process.exit(1)` so that the `/api/health` smoke endpoint stays
