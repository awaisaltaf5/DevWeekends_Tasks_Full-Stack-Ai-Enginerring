...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      // a seller may only create events for their own shop
      ...
      if (!shop) {
        ...
        const uploads = await Promise.all(
          files.map((file) => uploadBuffer(file.buffer, "vendora/events"))
        );
...
router.delete(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      // Delete the event's images from Cloudinary (no-ops for legacy/local paths)
      await Promise.all(
        (eventData.images || []).map((imageUrl) => deleteAsset(imageUrl))
      );
